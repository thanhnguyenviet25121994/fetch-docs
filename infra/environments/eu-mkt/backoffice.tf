data "aws_acm_certificate" "backoffice_acm" {
  domain   = "*.mkt.lnga-rg.com"
  statuses = ["ISSUED"]
  provider = aws.global
}

resource "aws_s3_bucket" "backoffice" {
  bucket = "bo.mkt.lnga-rg.com"

  tags = {
    Name        = "bo.mkt.lnga-rg.com"
    Environment = local.environment
  }
}

# resource "aws_s3_object" "backoffice_config" {
#   bucket  = aws_s3_bucket.backoffice.bucket
#   key     = "env-config.js"
#   content = <<EOT
# window._env_ = {
#   __API_BASE_URL__: "https://bo.mkt.lnga-rg.com/api"
# };
# EOT
# }
...
module "cloudfront_backoffice" {
  ...
  viewer_certificate = {
    acm_certificate_arn      = data.aws_acm_certificate.backoffice_acm.arn
