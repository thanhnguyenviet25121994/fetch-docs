...
module "rc_acm" {
  ...
  domain_name = aws_s3_bucket.rc.bucket
...
resource "cloudflare_record" "rc_acm" {
  for_each = {
    for dvo in module.rc_acm.validation_domains : dvo.domain_name => {
...
module "cloudfront_rc" {
  ...
  viewer_certificate = {
    acm_certificate_arn      = module.rc_acm.acm_certificate_arn
