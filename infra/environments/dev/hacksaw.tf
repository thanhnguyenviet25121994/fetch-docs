resource "aws_s3_bucket" "hacksaw" {
  bucket = "static.hacksaw.${local.environment}.${local.root_domain}"

  tags = {
    Name        = "revengegames-hacksaw-${local.environment}"
...
resource "cloudflare_record" "apihacksaw" {
  ...
  name    = "api.hacksaw.dev"
  type    = "CNAME"
  content = module.cloudfront_api_hacksaw.cloudfront_distribution_domain_name
  proxied = true
...
module "cloudfront_hacksaw" {
  ...
  comment = "CloudFront for Hacksaw Gaming clone (dev)"
...
### cloudfront api
module "cloudfront_api_hacksaw" {
  source  = "terraform-aws-modules/cloudfront/aws"
  version = "v3.4.0"

  aliases = ["api.hacksaw.${local.environment}.${local.root_domain}"]
  comment = "CloudFront for Hacksaw Gaming clone (dev)"
