resource "aws_s3_bucket" "hacksaw" {
  bucket = "static.hacksaw.sandbox.${local.root_domain}"

  tags = {
    Name        = "revengegames-hacksaw-sandbox"
...
module "cloudfront_hacksaw" {
  ...
  comment = "CloudFront for Hacksaw Gaming clone (sandbox)"
...
module "cloudfront_api_hacksaw" {
  ...
  aliases = ["api.hacksaw.sandbox.${local.root_domain}"]
  comment = "CloudFront for Hacksaw Gaming clone (sandbox)"
