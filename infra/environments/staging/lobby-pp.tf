resource "aws_s3_bucket" "lobby_pp" {
  bucket = "lobby.sandbox.pragmaticpplay.com"
...
module "cloudfront_lobby_pp" {
  ...
  comment = "CloudFront for lobby pp (sandbox)"
  ...
  viewer_certificate = {
    acm_certificate_arn      = "arn:aws:acm:us-east-1:211125478834:certificate/57f9d5dd-3694-4bef-a1b5-c68a3fa3aff5"
...
resource "aws_lb_listener_rule" "staging_lobby_pp" {
  ...
  action {
    ...
    fixed_response {
      ...
      message_body = jsonencode({
        baseApiUrl    = "https://api.sandbox.revenge-games.com",
        gameLaunchUrl = "https://sandbox.pragmaticpplay.com"
      })
      status_code = "200"
    }
  }
  ...
  condition {
    host_header {
      values = ["lobby.sandbox.pragmaticpplay.com"]
    }
  }

  tags = {
    Name        = "lobby.sandbox.pragmaticpplay.com"
    Environment = "${local.environment}"
  }
}
