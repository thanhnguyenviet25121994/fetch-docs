...
module "staging_lambda_service_logic" {
  ...
  alb_dns_name = aws_lb.staging_private.dns_name
  ...
  services = {
    ...
    # logic-lucky-snake-engine = {
    #   env = {
    #     NODE_ENV            = "production"
    #     DEFAULT_RTP_PROFILE = "B3"
    #     APP_ENV             = local.environment
    #     LOG_LEVEL           = "debug"
    #   }
    #   handler              = "build/src/server.handler"
    ...
    # logic-lucky-turtle-engine = {
    #   env                  = local.default_env_lambda_logic
    #   handler              = "build/src/server.handler"
    #   lambda_architectures = local.default_lambda_architectures
    #   memory_size          = local.default_memory_size
    # }
    logic-five-lions-megaways-clone = {
      env                  = local.default_env_lambda_logic
      handler              = "build/server.handler"
      lambda_architectures = local.default_lambda_architectures
      memory_size          = local.default_memory_size
    }
    # logic-chasing-leprechaun-coins-engine = {
    #   env                  = local.default_env_lambda_logic
    #   handler              = "build/src/server.handler"
    #   lambda_architectures = local.default_lambda_architectures
    #   memory_size          = local.default_memory_size
...
module "staging_lambda_service_logic_2" {
  ...
  alb_dns_name = aws_lb.staging_private2.dns_name
...
module "staging_lambda_service_logic_3" {
  ...
  alb_dns_name = aws_lb.staging_private3.dns_name
...
module "staging_lambda_service_logic_4" {
  source  = "../../modules/lambda-service-logic-3"
  app_env = local.environment
  network_configuration = {
    vpc_id = module.staging_networking.vpc.id
  ...
  alb_dns_name = aws_lb.staging_private4.dns_name
...
module "staging_lambda_deploy_notify" {
  ...
  ecs_cluster_arn = [
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-service-game-client",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-service-entity",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-service-pp-adaptor",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-operator-demo",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-portal-replay",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-share-ui-game-client",
    "arn:aws:ecs:ap-northeast-1:211125478834:cluster/staging-service-marketing-2",
  ]
}