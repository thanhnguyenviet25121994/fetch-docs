locals {
  alb_name            = regex("loadbalancer/(.+)", aws_lb.mkt_asia.arn)[0]
    ...
    [
      {
        ...
        treat_missing_data  = "missing"
        dimensions          = { InstanceId = "i-0148f2a1e6114c09e" }
        ...
        metric_query = [
          ...
          },
          {
            ...
            metric = {
              ...
              unit        = "Count"
              dimensions = {
                TargetGroup  = "targetgroup/${local.environment}-service-game-client/9c9c7a9adf5947db"
      ...
      ############ RDS Metrics ############
      {
        ...
        datapoints_to_alarm = "5"
        treat_missing_data  = "missing"
        dimensions          = { DBClusterIdentifier = aws_rds_cluster.mkt_asia_main.id }
...
module "mkt_asia_cloudwatch_sns_topic" {
  source = "../../modules/cloudwatch-sns-topic"

  region           = local.region
  environment      = local.environment
  sns_topic_name   = "${local.environment}-sns-topic-for-cloudwatch-alarms"
  LARK_WEBHOOK_URL = "https://botbuilder.larksuite.com/api/trigger-webhook/7f30b1ed295304c2e61d5128f5d59ccd"
}

module "mkt_asia_monitoring" {
  ...
  alarm_actions = [
    module.mkt_asia_cloudwatch_sns_topic.get_sns_topic_arn
  ]
  ok_actions = [
    module.mkt_asia_cloudwatch_sns_topic.get_sns_topic_arn
  ]
