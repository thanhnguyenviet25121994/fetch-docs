resource "aws_s3_bucket" "asg" {
  bucket = "asg.${local.rg_domain}"
  tags = {
    Name        = "allstar-${local.environment}"
    Environment = local.environment
  }
}

resource "aws_s3_bucket_website_configuration" "asg" {
...
resource "aws_s3_bucket_public_access_block" "asg_public" {
  bucket = aws_s3_bucket.asg.bucket
...
data "aws_iam_policy_document" "asg_s3_allow_public_access" {
  statement {
    principals {
      type        = "AWS"
      identifiers = ["*"]
    ...
    resources = [
      "${aws_s3_bucket.asg.arn}/*",
    ]
  }

  depends_on = [
    aws_s3_bucket_public_access_block.asg_public
  ]
}

resource "aws_s3_bucket_policy" "asg_allow_public_access" {
  bucket = aws_s3_bucket.asg.id
  policy = data.aws_iam_policy_document.asg_s3_allow_public_access.json
}


resource "cloudflare_record" "asg" {
  zone_id = data.cloudflare_zone.rg.id
  name    = "asg"
  type    = "CNAME"
  content = module.cloudfront_asg.cloudfront_distribution_domain_name
  proxied = true

  allow_overwrite = true
}

module "cloudfront_asg" {
  source              = "terraform-aws-modules/cloudfront/aws"
  version             = "v3.4.0"
  aliases             = [aws_s3_bucket.asg.bucket]
  comment             = "CloudFront for Allstars games (AS global, prod)"
  enabled             = true
  ...
  origin = {
    s3 = {
      domain_name = "${aws_s3_bucket_website_configuration.asg.website_endpoint}"
...
module "asg_log_bucket" {
  source  = "terraform-aws-modules/s3-bucket/aws"
  version = "~> 4.0"

  bucket = "allstar-global-logs"
