...
### service-game-crash
#############
module "prod_asia_service_game_crash" {
  ...
  network_configuration = {
    ...
    load_balancer_target_groups = [{
      arn  = aws_lb_target_group.prod_asia_service_game_crash.arn,
      port = 9300
    }]
  }
  ...
  depends_on = [
    ...
    aws_lb_listener_rule.prod_asia_service_game_crash
  ]
}

resource "aws_lb_target_group" "prod_asia_service_game_crash" {
...
resource "aws_lb_listener_rule" "prod_asia_service_game_crash" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 51

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.prod_asia_service_game_crash.arn
  }
...
module "prod_asia_service_gma_adaptor" {
  source = "../../modules/service-adaptor"
  ...
  network_configuration = {
    ...
    load_balancer_target_groups = [{
      arn  = aws_lb_target_group.prod_asia_service_gma_adaptor.arn,
      port = 9300
    }]
  }
...
resource "aws_lb_target_group" "prod_asia_service_gma_adaptor" {
...
resource "aws_lb_listener_rule" "prod_asia_service_gma_adaptor" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 1080

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.prod_asia_service_gma_adaptor.arn
  }
...
resource "cloudflare_record" "prod_asia_service_gma_adaptor" {
