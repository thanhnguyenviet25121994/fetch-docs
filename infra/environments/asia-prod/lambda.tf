...
module "prod_asia_lambda_service_logic" {
  source  = "../../modules/lambda-service-logic-global"
  app_env = local.environment
  ...
  alb_dns_name = aws_lb.prod_asia_private.dns_name
...
module "prod_asia_lambda_service_logic_2" {
  ...
  alb_dns_name = aws_lb.prod_asia_private2.dns_name
...
module "prod_asia_lambda_filter_log_to_lark" {
  ...
  lambda_env = {
    PROD_LARK_WEBHOOK_URL          = "https://open.larksuite.com/open-apis/bot/v2/hook/04cd31e8-6ad5-4882-a1ff-f9c594096ae5"
...
resource "aws_cloudwatch_log_subscription_filter" "sgc_lambda_destination" {
  name            = "ErrorFilter"
  log_group_name  = "prod-asia-service-game-client"
  filter_pattern  = "ERROR -\"o.s.w.reactive.socket.WebSocketHandler\" -\"Session mismatch\" -\"UNKNOWN\" -\"Missing bet setting\" -\"Game not found\""
  destination_arn = module.prod_asia_lambda_filter_log_to_lark.lambda_function_arn
