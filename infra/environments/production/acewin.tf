...
#############
module "prod_service_acewin_adaptor" {
  providers = {
    aws = aws.current
  }
  ...
  network_configuration = {
    ...
    load_balancer_target_groups = [{
      arn  = aws_lb_target_group.prod_service_acewin_adaptor.arn,
      port = 8080
    }]
  }
...
resource "aws_lb_target_group" "prod_service_acewin_adaptor" {
...
resource "aws_lb_listener_rule" "prod_service_acewin_adaptor" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 2220

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.prod_service_acewin_adaptor.arn
  }
