...
module "mkt_studio_cloudfront" {
  ...
  origin = {
    s3_mkt_studio = {
      ...
      # origin_access_control = "s3_mkt_studio"
    ...
    alb_mkt = {
      domain_name = "${aws_lb.mkt.dns_name}"
      custom_origin_config = {
        http_port              = 80
        https_port             = 443
        origin_protocol_policy = "http-only"
  ...
  default_cache_behavior = {
    target_origin_id             = "s3_mkt_studio"
  ...
  ordered_cache_behavior = [
    ...
    },
    {
      ...
      lambda_function_association = {
        ...
        origin-request = {
          lambda_arn = "arn:aws:lambda:us-east-1:211125478834:function:mkt-studio-edge-router:2"
        }
      }
    },
    {
...
resource "cloudflare_record" "mkt_studio" {
  zone_id = data.cloudflare_zone.root.id
  name    = "studio"
  content = module.mkt_studio_cloudfront.cloudfront_distribution_domain_name
  type    = "CNAME"
  proxied = false
}
