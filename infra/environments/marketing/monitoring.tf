locals {
  alb_name            = regex("loadbalancer/(.+)", aws_lb.mkt.arn)[0]
  cache_cluster_id    = "${local.environment}-service-game-client"
  dynamodb_table_name = "${local.environment}_bet_results"

  ecs_clusters = {
    "service-game-logic" = [
      # "mkt-logic-aztec-gems-clone",
      # "mkt-logic-doghouse-clone",
      # "mkt-logic-gates-of-olympus-1000-clone",
      # "mkt-logic-gates-of-olympus-clone",
      # "mkt-logic-jokers-jewels-clone",
      # "mkt-logic-star-light-princess-clone",
      # "mkt-logic-starlight-princess-1000-clone",
      # "mkt-logic-sweet-bonanza-1000-clone",
      # "mkt-logic-sweet-bonanza-clone",
      # "mkt-logic-sweet-bonanza-xmas-clone"
    ],
    ...
    [
      {
        ...
        treat_missing_data  = "missing"
        dimensions          = { InstanceId = "i-04411f616de7050b2" }
        ...
        metric_query = [
          ...
          },
          {
            ...
            metric = {
              ...
              unit        = "Count"
              dimensions = {
                TargetGroup  = "targetgroup/${local.environment}-service-game-client/1cdb84bea13b2f1b"
        ...
        metric_query = [
          ...
          },
          {
            ...
            metric = {
              ...
              unit        = "Count"
              dimensions = {
                TargetGroup  = "targetgroup/${local.environment}-service-pp-adaptor/329b339a721d79dd"
                LoadBalancer = "${local.alb_name}"
              }
            },
            label       = "HTTPCode_Target_5XX_Count",
      ...
      ############ RDS Metrics ############
      {
        ...
        datapoints_to_alarm = "5"
        treat_missing_data  = "missing"
        dimensions          = { DBClusterIdentifier = aws_rds_cluster.mkt_main.id }
...
module "mkt_cloudwatch_sns_topic" {
  source = "../../modules/cloudwatch-sns-topic"

  region           = local.region
  environment      = local.environment
  sns_topic_name   = "${local.environment}-sns-topic-for-cloudwatch-alarms"
  LARK_WEBHOOK_URL = "https://botbuilder.larksuite.com/api/trigger-webhook/dbec5c55823fe483945f2b94bcc72db4"
}

module "mkt_monitoring" {
  ...
  alarm_actions = [
    module.mkt_cloudwatch_sns_topic.get_sns_topic_arn
  ]
  ok_actions = [
    module.mkt_cloudwatch_sns_topic.get_sns_topic_arn
  ]
