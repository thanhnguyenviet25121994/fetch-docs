resource "aws_s3_bucket" "pg" {
  bucket = "payment.sandbox.revenge-games.com"

  tags = {
    Name        = "payment.sandbox.revenge-games.com"
...
module "sandbox_pg_cloudfront" {
  ...
  aliases = ["payment.sandbox.revenge-games.com", "demo-merchant.sandbox.revenge-games.com"]
  ...
  origin = {
    s3_sandbox_pg = {
      ...
      # origin_access_control = "s3_sandbox_pg"
  ...
  default_cache_behavior = {
    target_origin_id             = "s3_sandbox_pg"
    ...
    #   origin-request = {
    #     lambda_arn = "arn:aws:lambda:us-east-1:211125478834:function:sandbox-pg-edge-router:2"
    #   }
    # }
  }
  ...
  ordered_cache_behavior = [
    ...
    {
      path_pattern           = "/api/*"
      target_origin_id       = "alb_sandbox_pg"
      viewer_protocol_policy = "redirect-to-https"
...
resource "cloudflare_record" "sandbox_pg" {
  zone_id = data.cloudflare_zone.root.id
  name    = "payment.sandbox"
  content = module.sandbox_pg_cloudfront.cloudfront_distribution_domain_name
  type    = "CNAME"
  proxied = false
}

resource "cloudflare_record" "sandbox_pg_merchant" {
  zone_id = data.cloudflare_zone.root.id
