...
terraform {
  ...
  backend "s3" {
    ...
    key            = "prod/terraform.tfstate"       # Replace with the desired state file path (e.g., "prod/terraform.tfstate")
    region         = "ap-southeast-1"               # The region where your S3 bucket is located
    dynamodb_table = "pg-terraform-state-lock-prod" # DynamoDB table for state locking (optional but recommended)
