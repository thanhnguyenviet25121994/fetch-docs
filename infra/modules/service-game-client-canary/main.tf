...
module "container_definition_service_game_client" {
  ...
  environment = [for name, value in merge({
    "SERVER_PORT" : "9300",
    "SPRING_R2DBC_WRITE_URL" : "r2dbc:pool:postgresql://${var.db.endpoint}:5432/${var.db.name}",
  ...
  docker_labels = {
    ...
    # Define a router name
    "traefik.http.routers.service-game-client.rule"        = "Host(`sgctest.revenge-game.global`)"
    "traefik.http.routers.service-game-client.entrypoints" = "web"

    # Define the target service
    "traefik.http.services.service-game-client.loadbalancer.server.port" = "80"
  }
