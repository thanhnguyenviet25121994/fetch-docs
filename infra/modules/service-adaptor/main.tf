...
variable "network_configuration" {
  type = object({
    ...
    load_balancer_target_groups = optional(
      list(object({
        arn  = string
        port = number
      })),
...
module "ecs_svc_service" {
  ...
  container_definitions = {
    (var.app_name) = module.container_definition_service.container_definition
  }
  ...
  load_balancer = var.public_route ? {
  ...
  depends_on = [module.container_definition_service]
}
