...
variable "app_name" {
  type    = string
  default = "service-marketing-portal-adapter-2"
}
...
module "container_definition_service_marketing_portal_adapter" {
  ...
  secrets = [
    ...
    },
    {
      ...
      valueFrom = "${var.secrets.credentials.arn}:BACKOFFICE_CLIENTENV_LOBBYPAGEURL::"
    }
  ]
  log_configuration = {
    logDriver = "awslogs"
...
module "ecs_svc_service_marketing_portal_adapter" {
  ...
  container_definitions = {
    (var.app_name) = module.container_definition_service_marketing_portal_adapter.container_definition
  }
