...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  origin {
    ...
    origin_id   = "ALB_dev_Origin - /api/v1/client (config.json, env-config.js)"
  ...
  ordered_cache_behavior {
    ...
    smooth_streaming         = false
    target_origin_id         = "ALB_dev_Origin"
    viewer_protocol_policy   = "allow-all"
  ...
  ordered_cache_behavior {
    ...
    max_ttl                    = 0
    min_ttl                    = 0
    origin_request_policy_id   = "33f36d7e-f396-46d9-90e0-52428a34d9dc" # all viewer 2022-06
  ...
  tags = {
    ...
    Name        = "ALB dev CloudFront Distribution"
  }
}
...
#       aws cloudfront create-invalidation --distribution-id ${aws_cloudfront_distribution.s3_distribution.id} --paths "src/background.jpg"
#     EOT
#   }
