  ...
  dynamic "environment" {
    ...
    content {
      variables = merge(
        ...
        {
          APP_ENV   = var.app_env,
          LOG_LEVEL = contains(["dev", "staging"], var.app_env) ? "debug" : "error"
        }
      )
    }
  }
...
resource "aws_lambda_function_url" "services" {
  for_each           = var.app_env == "dev" ? { for s in var.services : s.name => s } : {}
...
resource "aws_lb_target_group" "services" {
  ...
  name        = trim(substr("lbd3-${var.app_env}-${each.key}-${random_string.random.result}", -32, -1), "-")
  target_type = "lambda"
...
#   for_each                          = var.app_env == "prod" ? { for s in var.services : s.name => s } : {}
#   function_name                     = aws_lambda_function.lambda_function[each.key].function_name
...
locals {
  service_order = [for s in var.services : s.name]
}

resource "aws_lb_listener_rule" "services" {
  for_each     = { for s in var.services : s.name => s }
  listener_arn = var.private_routes.load_balancer_listener.arn
  priority     = 610 + index(local.service_order, each.key) + 1
