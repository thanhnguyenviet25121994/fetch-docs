module "lambda_python_zip_notification" {
  ...
  lambda_python_zip_name    = "batch_failed_notification"
}
...
module "lambda_batch_failed_notification" {
  source = "./lambda"
  #basic
  env    = var.app_env
  ...
  #lambda
  lambda_function_name     = "batch-failed-notification"
...
module "eventbridge_lambda_batch_failed_notification" {
  source  = "terraform-aws-modules/eventbridge/aws"
  version = "v3.10.0"

  # bus_name = "${var.app_env}-${var.project_name}-batch-failed-notification"
  create_bus = false
  role_name  = "${var.app_env}-eb-${var.app_name}"


  rules = {
    "${var.app_env}-batch-failed-notification" = {
  ...
  targets = {
    "${var.app_env}-batch-failed-notification" = [
      {
        name = "lambda_batch_failed_notification"
        arn  = module.lambda_batch_failed_notification.lambda_function_arn
      }
    ]
  }
}
...
resource "aws_lambda_permission" "allow_eventbridge_lambda_batch_failed_notification" {
  statement_id  = "AllowExecutionFromEventBridge"
  action        = "lambda:InvokeFunction"
  function_name = module.lambda_batch_failed_notification.lambda_function_arn
  principal     = "events.amazonaws.com"

  # Use the EventBridge rule's ARN
  source_arn = module.eventbridge_lambda_batch_failed_notification.eventbridge_rule_arns["${var.app_env}-batch-failed-notification"]
}