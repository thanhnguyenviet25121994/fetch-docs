module "wildcard_cert" {
  ...
  domain_name = "*.${var.app_env}.${var.domain}"
  #   subject_alternative_names = [
  #     "*.${var.domain}"
  #   ]

  zone_id = var.cloudflare_id
...
resource "cloudflare_record" "validation" {
  count = length(module.wildcard_cert.distinct_domain_names)

  zone_id = var.cloudflare_id
  name    = element(module.wildcard_cert.validation_domains, count.index)["resource_record_name"]
  type    = element(module.wildcard_cert.validation_domains, count.index)["resource_record_type"]
  content = trimsuffix(element(module.wildcard_cert.validation_domains, count.index)["resource_record_value"], ".")
  ttl     = 60
  proxied = false
...
resource "aws_s3_bucket" "this" {
  bucket = "g.${var.app_env}.${var.domain}"

  tags = {
    Name        = "g.${var.app_env}.${var.domain}"
...
module "cloudfront" {
  ...
  aliases = ["g.${var.app_env}.${var.domain}"]

  comment             = "${var.app_env} Cloufront for game ${var.domain}"
  enabled             = true
  ...
  origin = {
    ...
    "alb_${var.app_env}_${var.app_name} - /api/v1/client (config.json, env-config.js)" = {
      domain_name = "${var.alb_dns_name}"
    ...
    "staging-1438152851.ap-northeast-1.elb.amazonaws.com https" = {
      domain_name = "${var.alb_dns_name}"
      custom_origin_config = {
        http_port              = 80
        https_port             = 443
  ...
  ordered_cache_behavior = [
    ...
    },
    {
      ...
      target_origin_id       = "alb_${var.app_env}_${var.app_name} - /api/v1/client (config.json, env-config.js)"
      viewer_protocol_policy = "allow-all"
    ...
    {
      path_pattern           = "/ws"
      target_origin_id       = "staging-1438152851.ap-northeast-1.elb.amazonaws.com https"
      viewer_protocol_policy = "allow-all"
...
resource "cloudflare_record" "staging_g_all_star" {
  zone_id = var.cloudflare_id
  name    = "g.${var.app_env}"
